/**
 * makeSquareTemplate — factory for category-specific square (1:1) carousel templates.
 *
 * All category templates share the same layout. Only the palette and metadata differ.
 * Layout constants match brand-square exactly.
 */
...
const IMG_W   = Math.round((W - PAD * 2) * 0.95 * 10) / 10;
const IMG_H   = Math.round(IMG_W * (510 / 1200) * 10) / 10;
...
export function makeSquareTemplate(name: string, description: string, C: Palette, fonts: FontDef[] = DEFAULT_FONTS): CarouselTemplate {
